// v3.5 — todos os caracteres especiais escapados
...
// í = \u00ed   ó = \u00f3   ú = \u00fa   ç = \u00e7   õ = \u00f5
// â = \u00e2   ê = \u00ea   à = \u00e0   ü = \u00fc
// Ã = \u00c3   Ç = \u00c7   Õ = \u00d5   É = \u00c9   Á = \u00c1   Ú = \u00da
// — = \u2014   » = \u00bb   º = \u00ba

var DEFENSORES = {
  "Dr. Robert Rios J\u00fanior": {
    lotacao: "2\u00aa Defensoria Itinerante",
    senha: "Robert2027"
  },
  "Dra. Andrea Melo de Carvalho": {
    lotacao: "1\u00aa Defensoria de Fam\u00edlia",
    senha: "Andrea2027"
  },
  "Dra. Dayana Sampaio Mendes Magalh\u00e3es": {
    lotacao: "2\u00aa Defensoria P\u00fablica Regional de Altos",
    senha: "Dayana2027"
  },
  "Dr. Eric Leonardo Pires de Melo": {
    lotacao: "7\u00aa Defensoria de Fam\u00edlia",
    senha: "Eric2027"
  },
  "Dra. L\u00edvia de Oliveira Revor\u00eado": {
    lotacao: "3\u00aa Defensoria P\u00fablica Regional de S\u00e3o Raimundo Nonato",
    senha: "Livia2027"
  },
  "Dr. Marcos Martins de Oliveira": {
    lotacao: "2\u00aa Defensoria de Floriano",
    senha: "Marcos2027"
  },
  "Dra. Priscila Gimenes do Nascimento Godoi": {
    lotacao: "2\u00aa Defensoria P\u00fablica Regional de Uni\u00e3o",
    senha: "Priscila2027"
  },
  "Dra. Julyanne Cristine Douglas Leone": {
    lotacao: "Assessora \u2014 2\u00aa Defensoria Itinerante",
    senha: "Julyanne2027"
  },
  "Dra. Giulia Mazza": {
    lotacao: "Assessora \u2014 2\u00aa Defensoria Regional de Piripiri",
    senha: "Giulia2027"
  }
};
    ...
    <div style={{ minHeight:"100vh", background:"#f0f2f0", display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ background:"#fff", borderRadius:12, padding:40, width:400, boxShadow:"0 8px 32px rgba(0,0,0,0.15)" }}>
        <div style={{ textAlign:"center", marginBottom:28 }}>
          ...
          <div style={{ fontWeight:800, fontSize:16, color:C.verde }}>
            {"Calculadora de D\u00e9bitos Alimentares"}
          </div>
          <div style={{ fontSize:12, color:"#888", marginTop:4 }}>
            {"Fase teste \u2014 Apenas Defensores Legais"}
          </div>
        </div>
        <div style={{ marginBottom:14 }}>
          <label style={{ display:"block", fontWeight:600, marginBottom:6, fontSize:13, color:C.cinza }}>
            {"Nome do Defensor"}
          </label>
          <select value={nome} onChange={function(e){setNome(e.target.value);}}
            ...
            {Object.keys(DEFENSORES).map(function(d,i){
              return <option key={i} value={d}>{d}</option>;
            })}
          </select>
          {nome && DEFENSORES[nome] && (
            <div style={{ fontSize:12, color:C.verde, marginTop:4, paddingLeft:4 }}>
              {"\u00bb "}{DEFENSORES[nome].lotacao}
            </div>
          )}
        </div>
        ...
        {erro && (
          <div style={{ background:"#fdecea", border:"1px solid #e57373", borderRadius:6, padding:"10px 12px", fontSize:12, color:C.vermelho, marginBottom:16 }}>
            {erro}
          </div>
        )}
        <button onClick={tentar}
          style={{ width:"100%", background:C.verde, color:"#fff", border:"none", borderRadius:6, padding:12, fontSize:15, fontWeight:700, cursor:"pointer", touchAction:"manipulation", marginBottom:10 }}>
          {"Entrar"}
        </button>
        <button onClick={onVisitante}
          style={{ width:"100%", background:"transparent", color:C.cinza, border:"1px solid " + C.borda, borderRadius:6, padding:10, fontSize:13, cursor:"pointer", touchAction:"manipulation" }}>
...
function Input(props) {
  ...
  return (
    <div style={{ marginBottom:14 }}>
      ...
      <input type={type} value={props.value}
...
function Card(props) {
  ...
  return (
    <div style={Object.assign({ background:C.branco, borderRadius:10, border:"1px solid " + C.borda, padding:24, marginBottom:20 }, style)}>
      {props.children}
    </div>
  );
}
...
function ModalPerfil(props) {
  ...
  var salvar = function() {
    ...
    props.onSave({ nome: nome, lotacao: lotacaoModal, apiKey: apiKey });
    props.onClose();
  };
  return (
        ...
        {nome && def && (
          <div>
            <div style={{ marginBottom:14 }}>
              ...
              <input value={def.lotacao} disabled
                style={{ width:"100%", padding:"9px 12px", borderRadius:6, border:"1px solid " + C.borda, fontSize:14, boxSizing:"border-box", background:C.cinzaClaro }} />
            </div>
            <div style={{ marginBottom:14 }}>
              <label style={{ display:"block", fontWeight:600, marginBottom:4, color:C.cinza, fontSize:13 }}>
                {"Senha"}
      ...
      <div style={{ display:"flex", alignItems:"center", gap:14 }}>
        ...
        <div>
          ...
          <div style={{ fontSize:12, opacity:.8 }}>
            {"APIDEP \u2014 Associa\u00e7\u00e3o Piauiense das Defensoras e Defensores P\u00fablicos"}
          </div>
        </div>
      </div>
      <div style={{ display:"flex", gap:8, alignItems:"center" }}>
        <button onClick={props.onPerfil}
          style={{ background:"rgba(255,255,255,0.15)", border:"1px solid rgba(255,255,255,0.4)", borderRadius:6, color:"#fff", padding:"7px 14px", cursor:"pointer", fontSize:13, touchAction:"manipulation" }}>
          {perfil.nome || "Visitante"}
        </button>
        <button onClick={props.onLogout}
          style={{ background:"rgba(255,255,255,0.1)", border:"1px solid rgba(255,255,255,0.3)", borderRadius:6, color:"#fff", padding:"7px 12px", cursor:"pointer", fontSize:12, touchAction:"manipulation" }}>
          {"Sair"}
        </button>
      </div>
    </div>
...
function AppInterno(props) {
  ...
  var salvarPerfil = function(p) {
    setPerfil(p);
    localStorage.setItem("dpe_perfil", JSON.stringify(p));
  };
  var addParcela = function() {
    setParcelas(function(p){ return p.concat([novaParcela()]); });
  };
  var removeParcela = function(id) {
    setParcelas(function(p){ return p.filter(function(x){ return x.id !== id; }); });
  };
  var editParcela = function(id, campo, val) {
    setParcelas(function(p){
      return p.map(function(x){
        if(x.id===id){ var n = Object.assign({}, x); n[campo]=val; return n; }
        return x;
      });
    });
  };
  var limparParcelas = function() {
  ...
  var addIntervalo = function() {
    ...
    while(a < intervalo.anoFim || (a === intervalo.anoFim && m <= intervalo.mesFim)){
      ...
      novas.push({
        id: Date.now()+novas.length, mes:m, ano:a, valor:valor,
        pago: intervalo.pago ? Number(intervalo.pago).toFixed(2) : "",
        is13: false
      });
      m++; if(m > 12){ m=1; a++; }
    }
      ...
      fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              block,
              { type:"text", text:"Extraia: n\u00famero do processo CNJ, alimentado, alimentante, parcelas. Responda SOMENTE em JSON: {\"processo\":\"\",\"alimentado\":\"\",\"alimentante\":\"\",\"parcelas\":[{\"mes\":1,\"ano\":2024,\"valor\":1500.00}]}" }
            ]
          }]
        })
        ...
        if(parsed.parcelas && parsed.parcelas.length)
          setParcelas(parsed.parcelas.map(function(p,i){
            return {id:Date.now()+i, mes:p.mes, ano:p.ano, valor:String(r2(p.valor)), pago:"", is13:false};
          }));
  ...
  var calcular = function() {
    if(!usuario.autenticado && !perfil.nome){
      alert("Essa calculadora \u00e9 somente para defensores legais.");
      return;
    }
    ...
    setTimeout(function(){
          ...
          if(doAno.length > 0){
            ...
            if(!ja13){
              ...
              if(dezVencido){
                ...
                parcelas13.push({
                  mes:13, ano:ano, label:"13\u00ba/"+ano,
                  smVig: getSM(12,ano), nominal:media, pago:0, is13:true
                });
              }
            }
          }
      ...
      var parcelasCorrigidas = raw.map(function(p, idx){
        ...
        var calc = quitado
          ? { fator:1, corrigido:0, juros:0, total:0, mesesAtraso:0 }
          : corrigir(saldoNom, p.mes, p.ano);
      ...
      var res = {
        ...
        percentualSM: percentualSM,
        valorFixoAlimento: valorFixoAlimento,
        justificativa: justFinal,
        prisao: prisaoItems,
        penhora: penhoraItems,
        totalPrisao: somaArr(prisaoItems),
        totalPenhora: somaArr(penhoraItems),
  ...
  var gerarPDF = function() {
    ...
    carregarLogo().then(function(logoData){
      ...
      if(logoData){
        try{
          var lh=22, lw=Math.max(lh*_logoRatio,30);
      ...
      if(resultado.justificativa){
        ...
        y += 5;
        doc.setDrawColor(26,107,58); doc.line(mg, y, W-mg, y);
        y += 4;
      ...
      var desenharTabela = function(titulo, corRGB, items, subtotal, numI){
        ...
        items.forEach(function(p,i){
          ...
          if(p.creditoAplicado > 0){
            doc.setTextColor(26,82,118); doc.setFont("helvetica","bold");
            doc.text(fmt(p.creditoAplicado), cx[6], y+2);
            doc.setTextColor(40,40,40); doc.setFont("helvetica","normal");
          } else {
            doc.text("-", cx[6], y+2);
          }
          ...
          if(p.quitado){
            doc.setTextColor(26,107,58); doc.text("QUITADO", cx[7], y+2);
          } else {
            doc.setTextColor(40,40,40); doc.text(fmt(r2(p.saldoBruto)), cx[7], y+2);
          }
          doc.setTextColor(40,40,40); doc.setFont("helvetica","normal");
          ...
          doc.setFont("helvetica","bold");
          if(p.quitado){
            doc.setTextColor(26,107,58); doc.text("-", cx[12], y+2);
          } else {
            doc.setTextColor(40,40,40); doc.text(fmt(p.total), cx[12], y+2);
          }
          y += 5.5;
        });
      ...
      doc.text("Observa\u00e7\u00f5es:", mg, y);
      y += 5;
      doc.setFont("helvetica","normal"); doc.setFontSize(7.5);
      var obs = [
        "1. Corre\u00e7\u00e3o monet\u00e1ria pelo IPCA (IBGE). \u00cdndices oficiais at\u00e9 fev/2026. A partir de mar/2026: proje\u00e7\u00e3o de 0,31% a.m. (m\u00e9dia dos \u00faltimos 6 meses oficiais). Sujeito a revis\u00e3o quando publicados os \u00edndices definitivos.",
      ...
      doc.text(resultado.defensor || "", W/2, y, {align:"center"});
      y += 5;
  ...
  var inpStyle = {
    ...
    border:"1px solid " + C.borda, fontSize:13, boxSizing:"border-box"
  };
  ...
  return (
    <div style={{ fontFamily:"Segoe UI, Arial, sans-serif", minHeight:"100vh", background:"#f0f2f0" }}>
      ...
      {showPerfil && (
        <ModalPerfil perfil={perfil} onSave={salvarPerfil} onClose={function(){setShowPerfil(false);}} />
      )}
          ...
          <div>
            {!perfil.nome && (
              <div style={{ background:"#fff8e1", border:"1px solid #f0c040", borderRadius:8, padding:"12px 18px", marginBottom:18, fontSize:14 }}>
                ...
                <span style={{ color:C.verde, cursor:"pointer", textDecoration:"underline" }}
                  onClick={function(){setShowPerfil(true);}}>
                  {"Configurar agora"}
                </span>
              </div>
            )}
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:20 }}>
              <Card style={{ margin:0, borderTop:"3px solid "+C.azul }}>
                ...
                {msgIA && (
                  <div style={{ marginTop:8, fontSize:12, color:msgIA.indexOf("OK")===0?C.verde:C.vermelho }}>
                    {msgIA}
                  </div>
                )}
                {!perfil.apiKey && (
                  <div style={{ marginTop:6, fontSize:11, color:"#999" }}>
                    {"Opcional. Requer chave API no perfil."}
                  </div>
                )}
              </Card>
            ...
            <Card>
              ...
              <div style={{ marginBottom:14 }}>
                <label style={{ display:"block", fontWeight:600, marginBottom:8, color:C.cinza, fontSize:13 }}>
                  {"Alimentos fixados em"}
                </label>
                  ...
                  {[["sm","% do Sal\u00e1rio M\u00ednimo"],["fixo","Valor fixo (R$)"]].map(function(item){
                    ...
                    return (
                      <button key={v} onClick={function(){setTipoAlimento(v);}} style={{
                        ...
                        border:"2px solid "+(tipoAlimento===v?C.verde:C.borda),
                        background: tipoAlimento===v ? C.verde : C.branco,
                        color: tipoAlimento===v ? "#fff" : C.cinza,
                        fontWeight:600, fontSize:13, cursor:"pointer", touchAction:"manipulation"
                      }}>{l}</button>
                    );
                  })}
                </div>
                {tipoAlimento==="sm"
                  ? (
                    <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                      <input type="text" inputMode="decimal" value={percentualSM}
                        onChange={function(e){setPercentualSM(e.target.value);}}
                        placeholder={"ex: 20"}
                        style={{ width:100, padding:"9px 12px", borderRadius:6, border:"1px solid "+C.borda, fontSize:14, boxSizing:"border-box" }} />
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                      ...
                      <input type="text" inputMode="decimal" value={valorFixoAlimento}
                        onChange={function(e){setValorFixoAlimento(e.target.value);}}
                        placeholder={"0,00"}
            ...
            <Card>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:16, padding:"12px 16px", background:incluir13?"#fff8e1":"#f9f9f9", border:"1px solid "+(incluir13?"#f0c040":C.borda), borderRadius:8 }}>
                <input type="checkbox" checked={incluir13}
                  onChange={function(e){setIncluir13(e.target.checked);}}
              ...
              <div>
                <label style={{ display:"block", fontWeight:600, marginBottom:4, color:C.cinza, fontSize:13 }}>
                  {"Justificativa / Observa\u00e7\u00f5es (aparece no PDF)"}
                </label>
                <textarea value={justificativa}
                  onChange={function(e){setJustificativa(e.target.value);}}
                  rows={4}
            ...
            <Card>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
                ...
                <div style={{ display:"flex", gap:8 }}>
                  ...
                  {parcelas.length > 0 && (
                    <Btn small onClick={limparParcelas} cor={C.vermelho} outline>{"Limpar"}</Btn>
                  )}
                </div>
              </div>
                ...
                <div style={{ background:"#f0f4f8", border:"1px solid "+C.azul, borderRadius:8, padding:16, marginBottom:16 }}>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:8, alignItems:"end" }}>
                    <div>
                      ...
                      <select value={intervalo.mesIni}
                        onChange={function(e){setIntervalo(Object.assign({},intervalo,{mesIni:Number(e.target.value)}));}}
                        style={inpStyle}>
                    ...
                    <div>
                      ...
                      <input type="number" value={intervalo.anoIni}
                        onChange={function(e){setIntervalo(Object.assign({},intervalo,{anoIni:Number(e.target.value)}));}}
                        style={inpStyle} />
                    </div>
                    <div>
                      <label style={{ fontSize:11, fontWeight:600, color:C.cinza, display:"block", marginBottom:3 }}>{"M\u00eas fim"}</label>
                      <select value={intervalo.mesFim}
                        onChange={function(e){setIntervalo(Object.assign({},intervalo,{mesFim:Number(e.target.value)}));}}
                        style={inpStyle}>
                        {MESES.map(function(m,idx){ return <option key={idx} value={idx+1}>{m}</option>; })}
                      </select>
                    </div>
                    <div>
                      ...
                      <input type="number" value={intervalo.anoFim}
                        onChange={function(e){setIntervalo(Object.assign({},intervalo,{anoFim:Number(e.target.value)}));}}
                        style={inpStyle} />
                    </div>
                    <div>
                      <label style={{ fontSize:11, fontWeight:600, color:C.cinza, display:"block", marginBottom:3 }}>{"Pago (R$)"}</label>
                      <input type="text" inputMode="decimal" value={intervalo.pago}
                        onChange={function(e){setIntervalo(Object.assign({},intervalo,{pago:e.target.value}));}}
                        placeholder={"0,00"} style={inpStyle} />
                    </div>
                  </div>
                  <div style={{ marginTop:12, display:"flex", gap:8, alignItems:"center" }}>
                    <Btn small onClick={addIntervalo} cor={C.azul}>
                      {"Adicionar " + contarParcelas() + " parcelas"}
                    </Btn>
              ...
              {parcelas.map(function(p){
                return (
                  <div key={p.id} style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr auto", gap:10, alignItems:"end", marginBottom:10, padding:12, background:p.is13?"#fff8e1":C.cinzaClaro, borderRadius:8, border:p.is13?"1px solid #f0c040":"none" }}>
                    <div>
                      <label style={{ fontSize:12, fontWeight:600, color:C.cinza, display:"block", marginBottom:3 }}>
                        {p.is13 ? "13\u00ba Sal\u00e1rio" : "M\u00eas"}
                      </label>
                      {p.is13
                        ? <div style={{ padding:8, fontSize:13, color:C.azul, fontWeight:700 }}>{"13\u00ba/" + p.ano}</div>
                        : (
                          <select value={p.mes}
                            onChange={function(e){editParcela(p.id,"mes",Number(e.target.value));}}
                            style={inpStyle}>
                            {MESES.map(function(m,idx){ return <option key={idx} value={idx+1}>{m}</option>; })}
                          </select>
                        )
                    ...
                    <div>
                      ...
                      <input type="number" value={p.ano}
                        onChange={function(e){editParcela(p.id,"ano",Number(e.target.value));}}
                        style={inpStyle} />
                    </div>
                    <div>
                      <label style={{ fontSize:12, fontWeight:600, color:C.cinza, display:"block", marginBottom:3 }}>{"Valor (R$)"}</label>
                      <input type="text" inputMode="decimal" value={p.valor}
                        onChange={function(e){editParcela(p.id,"valor",e.target.value);}}
                        placeholder={"0,00"} style={inpStyle} />
                    </div>
                    <div>
                      <label style={{ fontSize:12, fontWeight:600, color:C.cinza, display:"block", marginBottom:3 }}>{"Pago (R$)"}</label>
                      <input type="text" inputMode="decimal" value={p.pago}
                        onChange={function(e){editParcela(p.id,"pago",e.target.value);}}
                        placeholder={"0,00"} style={inpStyle} />
                    </div>
                    <button onClick={function(){removeParcela(p.id);}}
                      style={{ background:"transparent", border:"none", cursor:"pointer", color:C.vermelho, fontSize:18, paddingBottom:4, touchAction:"manipulation" }}>
                      {"\u2715"}
                    </button>
                  </div>
                );
              ...
              <Card style={{ borderLeft:"4px solid "+C.verde }}>
                ...
                {resultado.processo && (
                  <p style={{ margin:"0 0 4px", fontSize:13, color:"#666" }}>
                    {"Processo: "}<strong>{resultado.processo}</strong>
                  </p>
                )}
                {resultado.alimentado && (
                  <p style={{ margin:"0 0 16px", fontSize:13, color:"#666" }}>
                    {"Alimentado(a): "}<strong>{resultado.alimentado}</strong>
                  </p>
                )}
                {resultado.obsImputacao && (
                  <div style={{ background:"#fff8e1", border:"1px solid #f0c040", borderRadius:8, padding:"12px 16px", marginBottom:16, fontSize:12, color:"#555", lineHeight:1.6 }}>
                    <div style={{ fontWeight:700, color:"#b8860b", marginBottom:4, fontSize:13 }}>
                      {"Imputa\u00e7\u00e3o de Pagamentos (art. 354 CC)"}
                    </div>
                    {resultado.obsImputacao}
                    {resultado.creditoRemanescente > 0 && (
                      <div style={{ marginTop:8, fontWeight:700, color:C.verde }}>
                        {"Cr\u00e9dito remanescente: " + fmt(resultado.creditoRemanescente)}
                      </div>
                    )}
                  </div>
                )}
                {resultado.prisao.length > 0 && (
                  <div style={{ background:C.verdePale, border:"1px solid "+C.verde, borderRadius:8, padding:16, marginBottom:12 }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                      ...
                      <span style={{ background:C.verde, color:"#fff", borderRadius:20, padding:"3px 12px", fontSize:12, fontWeight:700 }}>
                        {fmt(resultado.totalPrisao)}
                      </span>
                    </div>
                      ...
                      return (
                        <div key={i} style={{ display:"flex", justifyContent:"space-between", fontSize:13, marginTop:4 }}>
                          <span>
                            {p.label}
                            {p.is13 ? " [13\u00ba]" : ""}
                            {p.pagoOriginal>0 ? (" (pago em ref.: "+fmt(p.pagoOriginal)+")") : ""}
                            {p.creditoAplicado>0 ? (" (cr\u00e9d.apl.: "+fmt(p.creditoAplicado)+")") : ""}
                            {p.quitado ? " QUITADO" : ""}
                          </span>
                ...
                {resultado.penhora.length > 0 && (
                  <div style={{ background:"#e8f0f8", border:"1px solid "+C.azul, borderRadius:8, padding:16, marginBottom:12 }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                      ...
                      <span style={{ background:C.azul, color:"#fff", borderRadius:20, padding:"3px 12px", fontSize:12, fontWeight:700 }}>
                        {fmt(resultado.totalPenhora)}
                      </span>
                    </div>
